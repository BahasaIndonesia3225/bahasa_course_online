...
create table login_device_info
(
    id         varchar(64) not null comment 'ID',
    member_id  varchar(64) null comment '会员ID',
    device_id  varchar(64) null comment '设备ID',
    login_time datetime    null comment '首次登录时间',
    constraint login_device_info_pk
        primary key (id)
)
    comment '登录设备';

alter table member
    add pay_status int null comment '支付状态';

alter table member
    add role int default 1 null comment '会员角色';

CREATE TABLE `question`  (
  `id` varchar(64) NOT NULL,
  `section_id` char(8) NULL COMMENT '小节ID',
  `content` varchar(4096) NULL COMMENT '题目内容',
...
CREATE TABLE `question_option`  (
  `id` varchar(64) NOT NULL,
  `question_id` varchar(64) NULL COMMENT '题目ID',
  `content` varchar(4096) NULL COMMENT '选项内容',
  `answer` int NULL COMMENT '是否答案(0 否 1 是)',
  `create_at` datetime NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NULL ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
);

CREATE TABLE `member_section_pass`  (
    `id` varchar(64) NOT NULL,
    `section_id` char(8) NULL COMMENT '小节ID',
    `member_id` char(8) NULL COMMENT '会员ID',
    `pass` int NULL COMMENT '是否通过(0 否 1 是)',
    `create_at` datetime NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` datetime NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`)
);

ALTER TABLE `chapter` ADD COLUMN `do_question` int NULL DEFAULT 0 COMMENT '是否需要做题(0 否 1 是)';

ALTER TABLE `member` ADD COLUMN `do_question` int NULL DEFAULT 0 COMMENT '是否需要做题(0 否 1 是)';

ALTER TABLE `login_device_info` ADD COLUMN `device_type` int NULL COMMENT '登录设备(1 安卓 2 IOS 3 Windows)';

ALTER TABLE `member` ADD COLUMN `device_limit_num` int NULL DEFAULT 2 COMMENT '设备限制数量';