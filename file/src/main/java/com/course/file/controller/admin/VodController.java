...
//        }
    ...
    @PostMapping("/video")
    public ResponseDto<FileDto> videoUpload(FileDto fileDto, MultipartFile file) {
        String filename = file.getOriginalFilename();
        if (StrUtil.isBlank(filename)) {
            throw new ValidatorException("上传文件不能为空！");
        }
        String use = fileDto.getUse();
        String key = fileDto.getKey();
        ...
        //如果文件夹不存在则创建
        String dir = useEnum.name().toLowerCase();
        String path = dir + File.separator + key + "." + suffix; // course\6sfSqfOwzmik4A4icMYuUe.mp4
        // 上传到vod
        String vod = "";
        String fileUrl = "";
        try {
            // 初始化VOD客户端并获取上传地址和凭证
            BaseResponse uploadVideoResponse = VodUtil.uploadStream(accessKeyId, accessKeySecret, filename, filename, file.getInputStream());
            vod = uploadVideoResponse.getVideoId();
            LOG.info("上传视频成功, vod : " + vod);
            DefaultAcsClient vodClient = VodUtil.initVodClient(accessKeyId, accessKeySecret);
            GetMezzanineInfoResponse response = VodUtil.getMezzanineInfo(vodClient, vod);
            LOG.info("获取视频信息, response : {}", JSON.toJSONString(response));
            ...
            LOG.info("上传视频失败, ErrorMessage : " + e.getLocalizedMessage(), e);
            throw new ValidatorException("文件上传失败！");
        }
        fileDto.setPath(path);
        fileDto.setVod(vod);
        fileService.save(fileDto);
